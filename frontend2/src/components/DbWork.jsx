...
export default function DbWork() {
  ...
  const fetchTablePreview = async (tableName) => {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.data) && result.data.length > 0) {
        ...
        setLocalTablePreviewError('Не удалось получить предпросмотр таблицы');
      }
    } catch (e) {
  ...
  const handleUploadToDb = async()=>{
    ...
    try{
      ...
      formData.append('db_schema', uploadSchema);
      ...
      setUploadStatus('error');
      setLocalTablePreviewError('Ошибка загрузки: '+err.message);
    }
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="upload">
          <div className="space-y-4">
            ...
            {uploadStatus==='error' && <div className="text-red-600 mt-2">Ошибка загрузки</div>}
          </div>
        </TabsContent>
      </Tabs>
    </div>
