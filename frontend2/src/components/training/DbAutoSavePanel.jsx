...
export default function DbAutoSavePanel(props) {
  ...
  const {
    ...
    setNewTableName,
    autoSaveSettings,
    setAutoSaveSettings, // <--- добавляем сюда
    handleDbConnect,
    handleDbInputChange,
    handleSchemaChange,
    handleTableChange,
  ...
  // Получаем trainFile из props или useData
  const trainFile = props.trainFile || useData().trainFile;
  // Логирование для отладки передачи файла
  // Логирование для отладки состояния автосохранения
  ...
  // --- Safe setter for selectedPrimaryKeys ---
  // Удаляем локальное состояние, работаем только с глобальным selectedPrimaryKeys

  // --- Debug: log after update ---
  const handlePrimaryKeyChange = (col, checked) => {
    let newKeys;
    if (checked) {
      if (!selectedPrimaryKeys.includes(col)) {
        newKeys = [...selectedPrimaryKeys, col];
      } else {
        newKeys = selectedPrimaryKeys;
      }
    } else {
      newKeys = selectedPrimaryKeys.filter(pk => pk !== col);
    }
    if (typeof setSelectedPrimaryKeys === 'function') {
      setSelectedPrimaryKeys(newKeys);
    }
  };
  ...
  return (
    <Card className="mt-0">
      ...
      {(autoSaveEnabled && autoSaveSettings) || autoSaveMenuOpen ? (
        <div
          style={{
            width: '100%',
        ...
        >
          <CardContent className="p-3">
            {autoSaveEnabled && autoSaveSettings && (
            ...
            {autoSaveMenuOpen && (
              <div className="space-y-3 border-t pt-3">
                {/* Убрали лишний текст */}
                <div className="font-medium mb-2">Подключение к базе данных</div>
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="w-full md:w-1/2 mt-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Array.isArray(fileColumns) && fileColumns.length > 0
                                ? fileColumns.map((col) => {
                                    const checked = Array.isArray(selectedPrimaryKeys) && selectedPrimaryKeys.includes(col);
                                    return (
                                      <label
                                        key={col}
                                        className={`flex items-center space-x-1 text-sm border rounded px-2 py-1 cursor-pointer transition-colors ${checked ? 'bg-blue-100 border-blue-500 font-semibold' : 'bg-gray-50 border-gray-300'}`}
                                      ...
                                      >
                                        <input
                                          ...
                                          checked={checked}
                                          onChange={e => {
                                            handlePrimaryKeyChange(col, e.target.checked);
                                          }}
                        ...
                        <div className="space-y-2">
                          <p className="text-sm text-muted-foreground">
                            {saveMode === 'existing' && selectedTable ? `Результаты будут сохранены в таблицу: ${selectedTable}` :
                              saveMode === 'create' && selectedSchema && newTableName ? `Будет создана новая таблица: ${selectedSchema}.${newTableName}` :
                              'Выберите схему и таблицу для сохранения результатов'}
                          </p>
                          <Button
                            onClick={async () => {
