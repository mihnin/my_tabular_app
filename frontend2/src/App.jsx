...
import DbWork from './components/DbWork';

// Protected Route wrapper component
function ProtectedRoute({ children, requiresTraining = false }) {
  const { trainData, predictData, trainingStatus } = useData()
  
  if (!trainData || !predictData) {
    return (
      <div className="p-6 max-w-7xl mx-auto">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">Данные не загружены</h1>
          <p className="text-muted-foreground mb-6">
            Для доступа к этой странице необходимо сначала загрузить оба набора данных (обучение и прогноз).
          </p>
          <Link to="/upload">
  ...
  return (
    <nav className="w-64 bg-sidebar border-r border-sidebar-border p-4 relative">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground">Прогнозирование табличных данных</p>
      </div>
      ...
      <div className="space-y-2">
        {navItems.map((item) => {
          ...
          // Для config, training, analysis: requiresFile -> оба датасета должны быть загружены (trainData и predictData)
          const isFileDisabled = item.requiresFile && (!trainData || !predictData)
          ...
          let disabledMessage = ''
          if (isFileDisabled) {
            disabledMessage = 'Требуются данные'
