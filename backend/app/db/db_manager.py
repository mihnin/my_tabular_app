...
@read_only_guard
async def create_table_from_df(df: pd.DataFrame, db_schema: str, table_name: str, username: str, password: str, primary_keys: list = None) -> None:
    """
    ...
    async with get_connection(username, password) as conn:
        ...
        """
        table_exists = await conn.fetchval(check_table_exists_query, db_schema, table_name)
        if table_exists:
        ...
        create_query = f'CREATE TABLE "{db_schema}"."{table_name}" ({columns_sql}{pk_sql})'
        await conn.execute(create_query)


async def _get_pk_columns(conn: asyncpg.Connection, db_schema: str, table_name: str) -> List[str]:
    """
    ...
    async with get_connection(username, password) as conn:
        if not df.empty:
            ...
            if update_on_pk:
                ...
                pk_columns = await _get_pk_columns(conn, db_schema, table_name)
            ...
            insert_query = f'INSERT INTO "{db_schema}"."{table_name}" ({columns_str}) VALUES ({values_template})'
...
async def get_table_rows(
    db_schema: str, table_name: str, username: str, password: str, limit: int | None = None
) -> list[dict]:
    """
    ...
    async with get_connection(username, password) as conn:
        ...
        valid_tables_query = """
            ...
            WHERE schemaname = $1
        """
        valid_tables = await conn.fetch(valid_tables_query, db_schema)
        valid_table_names = {row["tablename"] for row in valid_tables}

        if table_name not in valid_table_names:
            raise ValueError(f"Таблица '{table_name}' не найдена или недоступна в схеме '{db_schema}'.")
...
async def check_df_matches_table_schema(df: pd.DataFrame, db_schema: str, table_name: str, username: str, password: str) -> bool:
    """
    ...
    try:
        async with get_connection(username, password) as conn:
            ...
            """
            exists = await conn.fetchval(check_query, db_schema, table_name)
            if not exists:
                return False
            ...
            """
            db_columns = await conn.fetch(columns_query, db_schema, table_name)
