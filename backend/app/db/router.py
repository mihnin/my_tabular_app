...
async def get_table_preview(
    ...
    Тело запроса: {"db_schema": ..., "table": ...} или {"schema": ..., "table": ...}
    """
    db_schema = getattr(req, 'db_schema', None) or getattr(req, 'schema', None)
    table_name = req.table
    if not db_schema or not table_name:
        raise HTTPException(status_code=400, detail="db_schema (или schema) и table обязательны")
    try:
        result = await get_table_rows(db_schema, table_name, db_creds['username'], db_creds['password'], 5)
...
async def upload_excel_to_db_endpoint(
    ...
    try:
        ...
        if dbSaveMode == 'new':
            ...
            await create_table_from_df(df, db_schema, table_name, db_creds['username'], db_creds['password'], primary_keys=pk_list)
            await upload_df_to_db(df, db_schema, table_name, db_creds['username'], db_creds['password'])
...
async def download_table_from_db_endpoint(
    ...
    if not db_schema or not table_name:
        raise HTTPException(status_code=400, detail="db_schema (или schema) и table обязательны")
    try:
        result = await get_table_rows(db_schema, table_name, db_creds['username'], db_creds['password'])
...
async def save_prediction_to_db(
    ...
    Тело запроса: {"db_schema": ..., ...} или {"schema": ..., ...}
    """
    db_schema = getattr(req, 'db_schema', None) or getattr(req, 'schema', None)
    ...
    try:
        ...
        if create_new:
            await create_table_from_df(df, db_schema, table_name, db_creds['username'], db_creds['password'])
            await upload_df_to_db(df, db_schema, table_name, db_creds['username'], db_creds['password'])
        else:
            matches = await check_df_matches_table_schema(df, db_schema, table_name, db_creds['username'], db_creds['password'])
            if not matches:
...
async def create_table_from_file_endpoint(
    ...
    try:
        ...
        try:
            await create_table_from_df(df_first, db_schema, table_name, db_creds['username'], db_creds['password'], primary_keys=pk_list)
        except Exception as e:
            if "уже существует" in str(e) or "already exists" in str(e):
